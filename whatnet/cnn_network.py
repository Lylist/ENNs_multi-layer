...
class CnnNetwork(NetworkBase):
    ...
    def __convergence(self, data, target, is_predict=False):
        ...
        # 使用激发次数最多的作为结果,数量相等使用时间最早
        # senders -= min(self.__predictLayer['outputLayer'])
        # # print(senders, target)
        # if len(senders) != 0:
        #     min_times = [max(times) for i in range(10)]
        #     count = [0 for i in range(10)]
        #     for i, v_sender in enumerate(senders):
        #         count[v_sender] += 1
        #         min_times[v_sender] = min(min_times[v_sender], times[i])
        #     res_sender = 0
        #     min_time = max(times)
        #     for i, cnt in enumerate(count):
        #         if count[i] > count[res_sender]:
        #             res_sender = i
        #             min_time = min_times[i]
        #         elif count[i] == count[res_sender] and min_times[i] < min_time:
        #             res_sender = i
        #             min_time = min_times[i]
        #         else:
        #             pass
        #     return res_sender == target, senders, res_sender
        # else:
        #     return False, senders, -1
